...
async function runMultiFileTests() {
  ...
  const resumePdfPath = path.join(__dirname, '..', 'uploads', '7fcf4c85-689b-4d4e-95b1-6da8314e5a68.pdf');
  
  // Create a second test document if needed
  const secondFilePath = path.join(__dirname, '..', 'uploads', 'sample_notes.txt');
  if (!fs.existsSync(secondFilePath)) {
    fs.writeFileSync(secondFilePath, 'Sample Lecture Notes Document\nPage 1 Content\nSection 1: Data Structures');
  }

  // Upload File 1 (Resume PDF)
  const form1 = new FormData();
  form1.append('file', fs.createReadStream(resumePdfPath));
  const res1 = await axios.post(`${BACKEND_URL}/api/upload`, form1, {
  ...
  form2.append('file', fs.createReadStream(secondFilePath));
  const res2 = await axios.post(`${BACKEND_URL}/api/upload`, form2, {
  ...
  const authRes = await axios.post(`${BACKEND_URL}/api/auth/verify-otp`, {
    phone: '9988776655',
    otp: '123456',
    name: 'Alex Dmello',
  });
  ...
  const jobRes = await axios.post(`${BACKEND_URL}/api/jobs`, jobPayload);
  ...
  const payRes = await axios.post(`${BACKEND_URL}/api/payments/verify-simulated`, {
    jobId: createdJob.id,
  });
  ...
  for (let attempt = 1; attempt <= 10; attempt++) {
    ...
    const statusRes = await axios.get(`${BACKEND_URL}/api/jobs/${createdJob.id}`);
