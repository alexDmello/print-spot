...
async function runLiveVerification() {
  ...
  try {
    ...
    const checkSlugRes = await axios.get(`${BASE_URL}/api/admin/check-slug?slug=${testSlug}`, {
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    ...
    const onboardRes = await axios.post(`${BASE_URL}/api/admin/shops`, newShopPayload, {
      headers: { Authorization: `Bearer ${adminToken}` },
    });
    ...
    const shopLoginRes = await axios.post(`${BASE_URL}/api/shops/login`, {
    ...
    const meRes = await axios.get(`${BASE_URL}/api/shops/me`, {
    ...
    const publicRes = await axios.get(`${BASE_URL}/api/shops/${createdShop.slug}/public`);
    ...
    const toggleRes = await axios.put(
      `${BASE_URL}/api/shops/${createdShop.id}/toggle-open`,
      { is_open: false },
      { headers: { Authorization: `Bearer ${shopToken}` } }
    );
    ...
    const otpRes = await axios.post(`${BASE_URL}/api/auth/send-otp`, { phone: '9876500001' });
    const verifyUserRes = await axios.post(`${BASE_URL}/api/auth/verify-otp`, {
      phone: '9876500001',
    ...
    const payCounterRes = await axios.post(`${BASE_URL}/api/payments/pay-at-counter`, {
      jobId,
    ...
    const markPaidRes = await axios.post(`${BASE_URL}/api/queue/${createdShop.id}/mark-paid`, { jobId });
    console.log('✓ Mark Paid:', markPaidRes.data.message);

    const readyRes = await axios.post(`${BASE_URL}/api/queue/${createdShop.id}/ready`, { jobId });
    ...
    const pickupRes = await axios.post(`${BASE_URL}/api/jobs/${jobId}/pickup`, {
