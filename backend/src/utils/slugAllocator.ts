...
export async function allocateUniqueSlug(
...
): Promise<{ slug: string; isOriginalAvailable: boolean; suggestions: string[] }> {
  ...
  const taken = new Set<string>(res.rows.map((r: any) => r.slug.toLowerCase()));
  // Also include reserved slugs
  RESERVED_SLUGS.forEach(s => taken.add(s));
