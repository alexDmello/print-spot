...
async function runOnboardingAndDeviceTests() {
  ...
  try {
    ...
    const onboardRes = await axios.post(`${BACKEND_URL}/api/shops/onboard`, onboardPayload);
    ...
    console.log('   Default Services Seeded:', newShop.services?.map((s: any) => `${s.name} (₹${s.price})`));
    ...
    const addPrinterRes = await axios.post(`${BACKEND_URL}/api/shops/${shopId}/printers`, {
    ...
    const addedPrinter = addPrinterRes.data.printers.find((p: any) => p.name === 'High-Volume Backoffice Spooler');
    if (!addedPrinter) throw new Error('Added printer not found in shop printers list');
    ...
    const toggleRes = await axios.put(`${BACKEND_URL}/api/shops/${shopId}/printers/${addedPrinter.id}`, {
      status: 'offline',
    });
    const updatedPrinter = toggleRes.data.printers.find((p: any) => p.id === addedPrinter.id);
    console.log('   Updated printer status:', updatedPrinter.status);
    if (updatedPrinter.status !== 'offline') throw new Error('Device status toggle failed');
    ...
    const servicesRes = await axios.get(`${BACKEND_URL}/api/shops/${shopId}/services`);
    const servicesList = servicesRes.data;
    const colorService = servicesList.find((s: any) => s.name.toLowerCase().includes('color'));

    if (colorService) {
      // Toggle Color service OFF (e.g. Out of toner)
      const toggleServiceRes = await axios.put(`${BACKEND_URL}/api/shops/${shopId}/services/${colorService.id}`, {
        enabled: false,
      });
      const toggled = toggleServiceRes.data.services.find((s: any) => s.id === colorService.id);
      console.log(`   Toggled Color Service enabled state: ${toggled.enabled}`);
      if (toggled.enabled !== false) throw new Error('Failed to toggle Color service');
      console.log('   ✅ PASS: Shopkeeper successfully toggled Color service to disabled.');
    }

    // Add a custom service (e.g. Glossy Photo Paper)
    const addServiceRes = await axios.post(`${BACKEND_URL}/api/shops/${shopId}/services`, {
    ...
    const customService = addServiceRes.data.services.find((s: any) => s.name === 'Ultra Glossy Photo Sheet');
    ...
    const updateProfileRes = await axios.put(`${BACKEND_URL}/api/shops/${shopId}`, {
    ...
    const [kioskRes, adminRes, onboardPageRes] = await Promise.all([
      axios.get(`${FRONTEND_URL}/?shop=${shopId}`),
      axios.get(`${FRONTEND_URL}/admin?shop=${shopId}`),
      axios.get(`${FRONTEND_URL}/onboard`),
    ]);
