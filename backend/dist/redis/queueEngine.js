...
class QueueEngine {
    redis = null;
    inMemoryCounters = new Map();
    inMemoryQueues = new Map(); // shopId -> [jobId]
    constructor() {
        try {
            if (env_1.config.redisUrl) {
                console.log('[Queue] Connecting to Redis via REDIS_URL...');
                this.redis = new ioredis_1.default(env_1.config.redisUrl, {
     ...
     */
    async getNextTokenNumber(shopId) {
    ...
    getNextTokenMemory(shopId, dateKey) {
     ...
     */
    async enqueuePaidJob(shopId, jobId) {
    ...
    async dequeueJob(shopId, jobId) {
        ...
        await (0, db_1.query)('DELETE FROM queue_entries WHERE job_id = $1', [jobId]);
        await this.syncQueuePositions(shopId);
    }
    dequeueMemory(shopId, jobId) {
        const q = this.inMemoryQueues.get(shopId);
        if (q) {
     ...
     */
    async getJobPosition(shopId, jobId) {
     ...
     */
    async syncQueuePositions(shopId) {
        const jobIds = await this.getQueueJobIds(shopId);
        for (let i = 0; i < jobIds.length; i++) {
            const jId = jobIds[i];
            const pos = i + 1;
            await (0, db_1.query)(`INSERT INTO queue_entries (job_id, shop_id, position, status, updated_at)
         VALUES ($1, $2, $3, 'waiting', CURRENT_TIMESTAMP)
         ON CONFLICT (job_id) DO UPDATE SET position = $3, updated_at = CURRENT_TIMESTAMP`, [jId, shopId, pos]);
        }
    }
    /**
     * Manual override: Reorder a job in the queue (e.g. bump to position 1 or custom)
     */
    async reorderJob(shopId, jobId, newPosition) {
     ...
     */
    async getQueueSnapshot(shopId) {
        const jobIds = await this.getQueueJobIds(shopId);
        // Fetch details from print_jobs
        const jobsRes = await (0, db_1.query)(`SELECT j.id, j.user_id, j.shop_id, j.printer_id, j.file_name, j.file_size, j.page_count,
       ...
       ORDER BY j.token_number ASC`, [shopId]);
     ...
     */
    calculateWaitTime(position, pageCount = 5, jobsAhead = []) {
        if (position <= 1)
            return 1;
     ...
     */
    async getEstimatedWaitForJob(shopId, jobId) {
        const jobIds = await this.getQueueJobIds(shopId);
        const index = jobIds.indexOf(jobId);
        if (index <= 0)
            return 1;
        ...
        const res = await (0, db_1.query)(`SELECT page_count, settings FROM print_jobs WHERE id IN (${placeholders})`, aheadIds);
        return this.calculateWaitTime(index + 1, 5, res.rows);
    }
}
exports.queueEngine = new QueueEngine();
