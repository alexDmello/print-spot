...
async function runEndToEndTest() {
    ...
    const shopsRes = await axios_1.default.get(`${BASE_URL}/api/shops`);
    ...
    console.log(`✓ Connected printers: ${shop.printers.map((p) => p.name).join(', ')}`);
    ...
    const invalidFilePath = path_1.default.join(__dirname, 'test_unsupported.txt');
    fs_1.default.writeFileSync(invalidFilePath, 'This is an unsupported text document that should be rejected.');
    try {
        const form = new form_data_1.default();
        form.append('file', fs_1.default.createReadStream(invalidFilePath));
        await axios_1.default.post(`${BASE_URL}/api/upload`, form, { headers: form.getHeaders() });
        ...
        if (fs_1.default.existsSync(invalidFilePath))
    ...
    const samplePdfPath = path_1.default.join(__dirname, 'sample_test_doc.pdf');
    ...
    const formPdf = new form_data_1.default();
    formPdf.append('file', fs_1.default.createReadStream(samplePdfPath));
    const uploadRes = await axios_1.default.post(`${BASE_URL}/api/upload`, formPdf, { headers: formPdf.getHeaders() });
    ...
    const otpRes = await axios_1.default.post(`${BASE_URL}/api/auth/send-otp`, { phone: testPhone });
    ...
    const createJobRes = await axios_1.default.post(`${BASE_URL}/api/jobs`, {
        userId: user.id,
        shopId: shop.id,
    ...
    const payConfirmRes = await axios_1.default.post(`${BASE_URL}/api/payments/verify-simulated`, {
        jobId: job.id,
    });
    ...
    for (let attempt = 1; attempt <= 15; attempt++) {
        ...
        const statusRes = await axios_1.default.get(`${BASE_URL}/api/jobs/${job.id}`);
    ...
    const statsRes = await axios_1.default.get(`${BASE_URL}/api/shops/${shop.id}/stats`);
    ...
    const uploadedFilePathOnDisk = path_1.default.join(__dirname, '..', 'uploads', storedName);
    console.log(`   File before pickup exists on disk: ${fs_1.default.existsSync(uploadedFilePathOnDisk)}`);
    const pickupRes = await axios_1.default.post(`${BASE_URL}/api/jobs/${job.id}/pickup`, {
        pickupCode,
    });
    console.log('✓ Pickup response:', pickupRes.data);
    const fileExistsAfterPickup = fs_1.default.existsSync(uploadedFilePathOnDisk);
    ...
    if (fs_1.default.existsSync(samplePdfPath))
        fs_1.default.unlinkSync(samplePdfPath);
    console.log('\n=============================================================');
