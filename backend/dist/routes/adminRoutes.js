...
router.post('/login', async (req, res) => {
    try {
        ...
        const result = await (0, shopAuthService_1.loginAdmin)(username || '', password || '');
        res.json(result);
    }
    catch (err) {
...
router.get('/me', shopAuthService_1.adminAuthMiddleware, (req, res) => {
    res.json({ admin: req.admin });
});
// 3. Platform-Wide Analytics Dashboard
router.get('/analytics', shopAuthService_1.adminAuthMiddleware, async (_req, res) => {
    try {
        // Platform-wide totals
        const shopsTotalRes = await (0, db_1.query)(`
      SELECT 
      ...
      FROM shops
    `);
        const jobsSummaryRes = await (0, db_1.query)(`
      SELECT 
        COUNT(*) as total_jobs,
      ...
      FROM print_jobs
    `);
        const printersRes = await (0, db_1.query)(`
      SELECT 
    ...
    `);
        ...
        const topShopsRes = await (0, db_1.query)(`
      SELECT 
        s.id,
        s.name,
        s.location,
...
router.get('/shops', shopAuthService_1.adminAuthMiddleware, async (_req, res) => {
    try {
        const shopsRes = await (0, db_1.query)(`
      SELECT 
        s.*,
...
router.post('/shops', shopAuthService_1.adminAuthMiddleware, async (req, res) => {
    try {
        const { name, slug: customSlug, location, address, latitude, longitude, owner_name, owner_phone, owner_email, pin, password, opening_time, closing_time, working_days, upi_id, price_per_bw, price_per_color, spiral_price, staple_price, printer_name, printer_type, } = req.body;
        if (!name || !location) {
        ...
        const pwdHash = password ? (0, security_1.hashPassword)(password.trim()) : null;
        ...
        await (0, db_1.query)(`INSERT INTO shops (
        ...
        for (const s of initialServices) {
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [srvId, shopId, s.name, s.desc, s.cat, s.price, s.unit, s.enabled, s.is_default]);
        }
       ...
       VALUES ($1, $2, $3, $4, 'online', 'Microsoft Print to PDF')`, [`printer_${(0, uuid_1.v4)().substring(0, 8)}`, shopId, pName, pType]);
        const createdShopRes = await (0, db_1.query)('SELECT * FROM shops WHERE id = $1', [shopId]);
...
router.put('/shops/:id/status', shopAuthService_1.adminAuthMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await (0, db_1.query)('UPDATE shops SET is_active = $1 WHERE id = $2', [Boolean(is_active), id]);
        const io = (0, socketHandler_1.getSocketServer)();
        if (io) {
...
router.put('/shops/:id/credentials', shopAuthService_1.adminAuthMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const pwdHash = password ? (0, security_1.hashPassword)(password.trim()) : null;
        if (newPin && pwdHash) {
            await (0, db_1.query)('UPDATE shops SET pin = $1, password_hash = $2 WHERE id = $3', [newPin, pwdHash, id]);
        }
        else if (newPin) {
            await (0, db_1.query)('UPDATE shops SET pin = $1 WHERE id = $2', [newPin, id]);
        }
        else if (pwdHash) {
            await (0, db_1.query)('UPDATE shops SET password_hash = $1 WHERE id = $2', [pwdHash, id]);
        }
...
router.delete('/shops/:id', shopAuthService_1.adminAuthMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        await (0, db_1.query)('DELETE FROM shops WHERE id = $1', [id]);
        const io = (0, socketHandler_1.getSocketServer)();
        if (io) {
            io.emit('shop_fleet_updated');
        }
