...
async function runTests() {
    ...
    try {
        ...
        const putPricingRes = await axios_1.default.put(`${BACKEND_URL}/api/shops/${SHOP_ID}/pricing`, customPricing);
        ...
        if (Number(putPricingRes.data.shop.price_per_bw) === 2.50 &&
        ...
        const shopGetRes = await axios_1.default.get(`${BACKEND_URL}/api/shops/${SHOP_ID}`);
        ...
        if (printers.length >= 2) {
            ...
            const assignRes = await axios_1.default.post(`${BACKEND_URL}/api/shops/${SHOP_ID}/printers/assign`, {
                colorPrinterId,
                monoPrinterId,
            });
            console.log('   Response status:', assignRes.status);
            const updatedPrinters = assignRes.data.printers;
            const p1 = updatedPrinters.find((p) => p.id === colorPrinterId);
            const p2 = updatedPrinters.find((p) => p.id === monoPrinterId);
        ...
        const tempTextPath = path_1.default.join(__dirname, 'temp_test_doc.txt');
        fs_1.default.writeFileSync(tempTextPath, 'PrintSpot Automated Test Document\nLine 2: Universal file format testing.\nLine 3: Verified for direct printing.');
        const formData = new form_data_1.default();
        formData.append('file', fs_1.default.createReadStream(tempTextPath), {
            filename: 'sample_report.txt',
            contentType: 'text/plain',
        });
        const uploadRes = await axios_1.default.post(`${BACKEND_URL}/api/upload`, formData, {
        ...
        if (fs_1.default.existsSync(tempTextPath))
            fs_1.default.unlinkSync(tempTextPath);
        ...
        const frontendRes = await axios_1.default.get(`${FRONTEND_URL}/?shop=${SHOP_ID}`);
        ...
        const adminRes = await axios_1.default.get(`${FRONTEND_URL}/admin`);
