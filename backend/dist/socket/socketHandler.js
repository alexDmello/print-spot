...
function initSocketServer(httpServer) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('subscribe_job', async (data) => {
            ...
            try {
                const jobRes = await (0, db_1.query)('SELECT * FROM print_jobs WHERE id = $1', [data.jobId]);
                if (jobRes.rowCount > 0) {
                    ...
                    socket.emit('job_status_update', {
                        ...
                        estimatedWaitMinutes: queueEngine_1.queueEngine.calculateWaitTime(pos, job.page_count),
                    });
                }
            }
            catch (err) {
        ...
        socket.on('subscribe_shop', async (data) => {
            ...
            try {
                const snapshot = await queueEngine_1.queueEngine.getQueueSnapshot(data.shopId);
                socket.emit('queue_updated', snapshot);
                const printersRes = await (0, db_1.query)('SELECT * FROM printers WHERE shop_id = $1', [data.shopId]);
        ...
        socket.on('p2p_ice_candidate', (data) => {
        ...
        socket.on('agent_register', async (data) => {
            if (data.agentKey !== env_1.config.printerAgentKey) {
            ...
            if (data.printers && Array.isArray(data.printers)) {
                for (const p of data.printers) {
                    await (0, db_1.query)(`UPDATE printers SET status = $1, system_name = $2 WHERE shop_id = $3 AND id = $4`, [p.status || 'online', p.systemName || p.name, data.shopId, p.id]);
                }
            }
        ...
        socket.on('agent_job_status', async (data) => {
            ...
            try {
                ...
                if (status === 'printing') {
                    await (0, db_1.query)(`UPDATE print_jobs SET status = 'printing', error_message = NULL WHERE id = $1`, [jobId]);
                }
             ...
             WHERE id = $2`, [errorMessage || 'Printer reported hardware or communication error', jobId]);
                    // Flag in queue
                    await (0, db_1.query)(`UPDATE queue_entries SET status = 'failed' WHERE job_id = $1`, [jobId]);
                }
                ...
                const pos = await queueEngine_1.queueEngine.getJobPosition(shopId, jobId);
        ...
        socket.on('agent_printer_heartbeat', async (data) => {
            try {
                for (const p of data.printers) {
                    await (0, db_1.query)(`UPDATE printers SET status = $1 WHERE shop_id = $2 AND (id = $3 OR name = $4)`, [p.status, data.shopId, p.id, p.name]);
                }
                const printersRes = await (0, db_1.query)('SELECT * FROM printers WHERE shop_id = $1', [data.shopId]);
 ...
 */
async function checkAndDispatchNextJob(shopId) {
    try {
        // Check if there is already a job actively printing for this shop
        const currentPrinting = await (0, db_1.query)(`SELECT id FROM print_jobs WHERE shop_id = $1 AND status = 'printing'`, [shopId]);
        if (currentPrinting.rowCount > 0) {
            // Printer is currently busy with a job
            return false;
        }
        // Get ordered waiting job IDs
        const jobIds = await queueEngine_1.queueEngine.getQueueJobIds(shopId);
        if (jobIds.length === 0)
            return false;
        // Pick the first waiting job
        const nextJobId = jobIds[0];
        const jobRes = await (0, db_1.query)(`SELECT j.*, s.name as shop_name 
       FROM print_jobs j 
       JOIN shops s ON j.shop_id = s.id 
       WHERE j.id = $1 AND j.status = 'waiting'`, [nextJobId]);
        if (jobRes.rowCount === 0)
            return false;
        const job = jobRes.rows[0];
        ...
        const routingDecision = await (0, printerRoutingService_1.assignPrintersToJob)(shopId, settings, job.page_count, job.file_url, job.file_name);
        ...
        if (assignedPrinterId) {
            await (0, db_1.query)(`UPDATE print_jobs SET printer_id = $1 WHERE id = $2`, [assignedPrinterId, job.id]);
        }
