...
async function runTests() {
    ...
    assert_1.default.strictEqual(detectRes.status, 200, 'detectRes status should be 200');
    const detectData = await detectRes.json();
    assert_1.default.strictEqual(detectData.success, true, 'detectData should succeed');
    (0, assert_1.default)(Array.isArray(detectData.printers), 'printers should be an array');
    (0, assert_1.default)(detectData.printers.length > 0, 'should detect at least 1 system printer');
    console.log(`   ✅ Successfully detected ${detectData.printers.length} host system printer(s):`);
    detectData.printers.forEach((p) => console.log(`      - ${p.name} (${p.driver}) [${p.status}]`));
    ...
    assert_1.default.strictEqual(assignMonoRes.status, 200, 'assignMono status should be 200');
    const assignMonoData = await assignMonoRes.json();
    assert_1.default.strictEqual(assignMonoData.success, true);
    const foundMono = assignMonoData.printers.find((p) => p.system_name === targetPrinter.system_name);
    (0, assert_1.default)(foundMono, 'Assigned printer should be in shop printers');
    assert_1.default.strictEqual(foundMono.type, 'mono');
    ...
    assert_1.default.strictEqual(assignColorRes.status, 200, 'assignColor status should be 200');
    const assignColorData = await assignColorRes.json();
    assert_1.default.strictEqual(assignColorData.success, true);
    const foundColor = assignColorData.printers.find((p) => p.system_name === targetPrinter.system_name);
    assert_1.default.strictEqual(foundColor.type, 'color');
    ...
    assert_1.default.strictEqual(jobRes.status, 201, 'job creation status should be 201');
    const jobData = await jobRes.json();
    (0, assert_1.default)(jobData.job.id, 'Job ID should be created');
    ...
    assert_1.default.strictEqual(payRes.status, 200, 'payRes status should be 200');
    const payData = await payRes.json();
    (0, assert_1.default)(payData.tokenCode, 'Token code should be assigned');
